...
const baseQueryWithRefetching = async (args, api, extraOptions) => {
  ...
  if (result.error) {
    console.error(`Произошла ошибка: ${result?.error?.originalStatus
    } Идентификатор ошибки ${result?.error?.data}`)
    const refetchResult = await baseQuery(args, api, extraOptions)
    if (refetchResult.data) {
      result = refetchResult
    } else {
      console.error("Сервер не отвечает, обновите страницу или попробуйте позже")
      return
    }
  }
  return result
